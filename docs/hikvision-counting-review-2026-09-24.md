...
The gateway diagnostic from build `a17c721` established that this camera advertises `regionTargetNumberCounting`, `supportSearchReport=true`, HTTP event-host configuration, and three currently empty HTTP host slots. The implementation therefore adds a bounded `/hikvision/events` receiver and a guarded camera-slot configuration action. Hikvision firmware families use different subscription names for counting, so the camera is asked for XML event metadata for the selected channel and the receiver retains counting events only. The receiver accepts only the configured camera address, limits request size, and never stores image parts. The selected slot is backed up before a PUT is attempted. This still does not label counts as verified until an actual camera message and controlled crossing test have been observed.
